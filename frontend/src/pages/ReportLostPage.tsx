...
export const ReportLostPage: React.FC = () => {
  ...
  const handleSubmit = async (data: ItemFormData) => {
    try {
      ...
      // Redirect immediately to the item's detail page showing AI-generated matches
      navigate(`/item/${result.item.id}`);
    } catch (err: any) {
      console.error('Submission failed:', err);
      setSubmitError(
        err.message || 'An error occurred while submitting your lost item report.'
      );
    } finally {
